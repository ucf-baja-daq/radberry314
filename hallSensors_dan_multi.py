...
# Hall sensor collection
# Strain gauge collction - calls a C program
	...
	def run(self, q, useQueue):
		...
		while self.runningFlag == 1:
			...
			if self.input_hallSen == self.isHallSenWithBoard and self.hallFlag == 0:
				...
				# store current time in t2 and move previous time into t1
				self.t1, self.t2 = self.t2, self.curTime
